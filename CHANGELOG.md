...
- Voice capture 只读取 ChatVoice list metadata endpoint，payload 保留 talk ID、标题、tags、时间、时长和摘要/转写可用性布尔值，不保存 summary/transcript/preview 正文或 token。
- Voice incremental capture 显式比较上一条已存事件的 tag snapshot；即使 ChatVoice 的 `updated_at` 没变化，tag 变化也会生成 `talk.updated`，并通过 metadata fingerprint 防止被旧 dedupe key 吞掉。
...
- `chatevent --tree` 改为由 ChatStyle 渲染的标准 CLI 树，保留现有 argparse 执行路径，避免破坏已发布命令行为。
- 新增 `chatevent.config:ChatEventConfig` 并注册到 ChatEnv `chatenv.configs` entry point；ChatEvent 运行态/API/登录 bootstrap ENV 交给 ChatEnv profile/secret 管理。
- 默认 ChatArch home 解析优先使用 ChatEnv `get_paths().home_dir`，同时保留 `CHATARCH_HOME`、`CHATEVENT_DB` 和旧路径迁移兼容。
- MkDocs 导航改为标准分组，新增 `CLI 树` 与 `接口树` 中英文页面，并同步 README/reference 的接口说明。
...
- 新增轻量登录与用户管理骨架（0.1.5 已修正为账号密码网页登录 + API Token 机制）：`/api/session`、`/api/users`、CLI `api create-user`，token 只返回一次且服务端只存 hash。
