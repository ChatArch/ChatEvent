...
class CliTests(unittest.TestCase):
    def test_tree_lists_observatory_capture_and_schema_commands(self) -> None:
        ...
        self.assertIn("│   ├── subscription-once [--db DB]", tree)
        self.assertIn("│   ├── voice-backfill [--db DB]", tree)
        self.assertIn("│   ├── voice-once [--db DB]", tree)
        self.assertIn("│   ├── watch-zulip-topic [--db DB]", tree)
    ...
    def test_tree_brief_omits_signatures_but_keeps_commands(self) -> None:
        ...
        self.assertIn("│   ├── subscription-once  # Run one capture pass for a saved subscription.", tree)
        self.assertIn("│   ├── watch-zulip-topic  # Create a temporary Zulip stream/topic watch", tree)
