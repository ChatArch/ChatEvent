...
`chatevent --tree` and `chatevent --tree-brief` are rendered by ChatStyle. `chatevent.config:ChatEventConfig` is registered under the ChatEnv `chatenv.configs` entry point. ChatEvent only declares Event Hub env keys: `CHATEVENT_API_URL`, `CHATEVENT_DB`, `CHATEVENT_ADMIN_TOKEN(_FILE)`, `CHATEVENT_API_USERNAME`, `CHATEVENT_API_PASSWORD_FILE`, `CHATEVENT_BOOTSTRAP_USERNAME`, `CHATEVENT_BOOTSTRAP_PASSWORD_FILE`, `CHATEVENT_PUBLIC_ORIGIN`, `CHATEVENT_COOKIE_SECURE`, `CHATEVENT_SESSION_TTL_SECONDS`, and `CHATEVENT_MAX_SESSIONS`.
...
The Web Observatory `Subscriptions` tab supports creating, editing, enabling/disabling, and deleting subscriptions through the same REST API. If users or bootstrap credentials are configured, visits to `/` first show the default ChatLogin username/password page and the Observatory is shown only after login; event-stream, stats, platform catalog, schema, and subscription read APIs also require login. Browser sessions use ChatLogin `SessionManager` with TTL, bounded capacity, login rotation, and logout revocation; every protected request re-reads the current EventStore enabled state and role. Cookie-authenticated browser writes carry CSRF. CLI, model, or programmatic clients can use `X-ChatEvent-Admin-Token` with the account's `arch_xxx` API token or the legacy bootstrap admin token; these validated API requests are CSRF-exempt, while a forged token header does not bypass cookie CSRF.
